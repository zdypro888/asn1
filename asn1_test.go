...
func TestUnexportedStructField(t *testing.T) {
	// 非导出字段应被静默跳过 (类似 encoding/json 的行为)
	bs, err := Marshal(unexported{X: 5, y: 1})
	if err != nil {
		t.Fatalf("Marshal unexported struct failed: %v", err)
	}

	// 验证只编码了导出字段 X
	bs2, err := Marshal(exported{X: 5, Y: 0})
	if err != nil {
		t.Fatal(err)
	}
	// unexported{X:5, y:1} 应和 exported{X:5, Y:0} 编码一致 (只有 X=5)
	// 注: exported 有两个导出字段，所以结果不同; 改用单字段结构验证
	var u unexported
	_, err = Unmarshal(bs, &u)
	if err != nil {
		t.Fatalf("Unmarshal unexported struct failed: %v", err)
	}
	if u.X != 5 {
		t.Errorf("got X=%d, want 5", u.X)
	}
	_ = bs2 // 不需要比较，只验证不报错
}
